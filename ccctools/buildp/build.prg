...
static s_objdir:=getenv("BUILD_OBJ")
static s_pobjdir:=dirsep()+curdir()+dirsep()+s_objdir
static s_curdir

#define MAXTHREAD  4
static mutex_th:=thread_mutex_init()
...
local d1,f,o,n,i,txt,dep
    ...
    for n:=1 to len(dir)
        ...
        //?? "forrás directoryk listája"
        ?? "",dir[n]::strtran(s_curdir+dirsep(),"")
    ...
    for n:=1 to len(todo)
        thread_mutex_lock(mutex_th)
        ++thread_count
        while( thread_count>MAXTHREAD )
            thread_cond_wait(cond_th,mutex_th)
        end
        thread_mutex_unlock(mutex_th)

        if( s_dry )
            makeobj(todo[n])
        else
            th:=thread_create({|i| sleep(i*0), makeobj(todo[i])},n)
...
local n,p1,p2
    ...
    thread_mutex_lock(mutex_th)
    --thread_count
    thread_cond_signal(cond_th)
    thread_mutex_unlock(mutex_th)

 

****************************************************************************
static function runx(cmd)

static mutex_out:=thread_mutex_init()
local torun:='bash -c "mkdir -p ../NAME && cd ../NAME && (CMD NAME DIR >out)"'
local out,error

    torun::=strtran("CMD",cmd[1])
    torun::=strtran("NAME",cmd[2])
    torun::=strtran("DIR",cmd[3])
    //?? torun;?
    run( torun )
 
    if( file("../"+cmd[2]+"/out") )
        out:=memoread("../"+cmd[2]+"/out")
        out::=strtran(s_curdir+dirsep(),"")
        out::=strtran(s_curdir+chr(10),"."+chr(10))
    ...
    if( file("../"+cmd[2]+"/error") )
        error:=memoread("../"+cmd[2]+"/error")
        thread_mutex_lock(mutex_out)
        ?? error
        thread_mutex_unlock(mutex_out)
        memowrit("../../error-"+cmd[2],error)
        quit
    else
        run("rm -f  ../../error-"+cmd[2] )
    end

    
    torun:=<<RUN>>bash -c "
    (mv -f  ../NAME/OBJDIR/* POBJDIR 2>/dev/null);
    (mv -f  ../NAME/ppo/* ../../ppo  2>/dev/null);
     rm -fr ../NAME 
    "<<RUN>>


    torun::=strtran("POBJDIR",s_pobjdir)
    torun::=strtran("OBJDIR",s_objdir)
    torun::=strtran("NAME",cmd[2])
    torun::=strtran(chr(10),"")
    //?? torun;?
    run( torun )
