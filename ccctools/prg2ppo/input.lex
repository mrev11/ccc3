...
%x st_ifdef st_ifndef st_ifeq st_ifneq
%x st_elifdef st_elifndef st_elifeq st_elifneq
%x st_waitelse st_waitendif
%x st_incl
%x st_comment
%x st_raw st_waitraw
...
^[ \t]*#else[ \t]*ifdef      {assert_open();ifdef_state(st_waitendif);}
^[ \t]*#else[ \t]*ifndef     {assert_open();ifdef_state(st_waitendif);}
^[ \t]*#else[ \t]*ifeq       {assert_open();ifdef_state(st_waitendif);}
^[ \t]*#else[ \t]*ifneq      {assert_open();ifdef_state(st_waitendif);}
^[ \t]*#else                 {assert_open();ifdef_state(st_waitendif);}
^[ \t]*#endif                {ifdef_pop();}
...
<st_elifdef>{
[ \t]
{SYMBOL}{EOL}                {procifdef(4);}
{SYMBOL}{EOLX}               {procifdef(4);statepush(st_comment);}
...
<st_elifndef>{
[ \t]
{SYMBOL}{EOL}                {procifdef(5);}
{SYMBOL}{EOLX}               {procifdef(5);statepush(st_comment);}
(.|\n)                       error("Syntax ERROR in #else ifndef directive");
}

<st_elifeq>{
[ \t]
{SYMBOL}{EOL}                {procifdef(6);}
{SYMBOL}{EOLX}               {procifdef(6);statepush(st_comment);}
...
<st_elifneq>{
[ \t]
{SYMBOL}{EOL}                {procifdef(7);}
{SYMBOL}{EOLX}               {procifdef(7);statepush(st_comment);}
...
<st_waitelse>{
^[ \t]*#else[ \t]*ifdef      {assert_open();statepush(st_ifdef);}
^[ \t]*#else[ \t]*ifndef     {assert_open();statepush(st_ifndef);}
^[ \t]*#else[ \t]*ifeq       {assert_open();statepush(st_ifeq);}
^[ \t]*#else[ \t]*ifneq      {assert_open();statepush(st_ifneq);}
^[ \t]*#else                 {assert_open();ifdef_state(0);}
}


<st_waitelse,st_waitendif>{
^[ \t]*#endif                {ifdef_pop();}
^[ \t]*#ifdef                {ifdef_push();ifdef_state(st_waitendif);}
^[ \t]*#ifndef               {ifdef_push();ifdef_state(st_waitendif);}
^[ \t]*#ifeq                 {ifdef_push();ifdef_state(st_waitendif);}
^[ \t]*#ifneq                {ifdef_push();ifdef_state(st_waitendif);}
{STRING}
{COMMENT1}
{COMMENTML}                  statepush(st_comment); //többsoros
