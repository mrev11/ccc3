...
//mindenesetre mutatja, mit tárol egy list_store.
...
local types:="",t
    ...
    store:=gtk.list_store.new(types)
    for i:=1 to length
        gtk.list_store.append(store,iter)
        gtk.list_store.set(store,iter,a[i])
    end
    gtk.tree_iter.free(iter)
    return store
