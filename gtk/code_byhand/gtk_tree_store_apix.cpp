...
// Pl.   gtk.tree_store.new({"B","I","S"})  //Boolean, Int, String
// vagy  gtk.tree_store.new("B","I","S")  //Boolean, Int, String
// vagy  gtk.tree_store.new("BIS")  //Boolean, Int, String
//
// Az első két paraméterezést visszavezetjük a harmadikra.

{
    VALUE *base=stack-argno;
    push_call("gtk.tree_store.new",base);
    ...
    GType *gtype=(GType*)malloc(n_columns*sizeof(GType)); //fasz MS
    ...
    gpointer ret=gtk_tree_store_newv(n_columns,gtype);
    free(gtype);
    _retp(ret);
    pop_call();
}
                        ...
                        VALUE *base, int argno, int i, VALUE v)
{
    ...
    else if( v.type==TYPE_BINARY )
    {
        if( ct==G_TYPE_STRING )
        {
            ...
            gtk_tree_store_set(GTK_TREE_STORE(self),iter,i,value,-1);
        }
        else
        {
            ARGERROR();
...
1)  gtk.tree_store.set(self,iter,{val1,val2,...})
...
2)  gtk.tree_store.set(self,iter,val1,val2,...)
    
    Ez ugyanaz mint az előző eset, vissza van rá vezetve.

3)  gtk.tree_store.set(self,iter,{val1, {idx,val2}, ...})
...
Megjegyzés:
    ...
    VALUE *base=stack-argno;
    push_call("gtk.tree_store.set",base);
...
{
    CCC_PROLOG("gtk.tree_store.set_value",4);
    gpointer self=_parp(1);
    GtkTreeIter* iter=(GtkTreeIter*)_parp(2);
