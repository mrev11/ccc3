...
local treeview  //GtkWidget
    ...
    window:=gtkwindowNew()
    window:set_title("GtkTreeStore Editable")
...
local renderer2:=gtkcellrenderertextNew()
    ...
    renderer2:signal_connect("edited",{|r,p,t|cb_edited12(r,p,t)})
    return column
...
static function cb_edited2(renderer,pathstr,text)
local iter:=gtk.tree_iter.new(),num
    if( model:get_iter_from_string(iter,pathstr) )
        num:=val(text)
        if( 0<num .and. num<=len(stock_ids) )
            model:set_value(iter,1,val(text))
        end
    end
    gtk.tree_iter.free(iter)
    //? renderer, pathstr, text
...
******************************************************************************
static function dump(model,iter)
...
local positioned
    ...
    if( iter==NIL )
        iter1:=gtk.tree_iter.new()
        positioned:=model:get_iter_first(iter1)
    else
        iter1:=iter
        positioned:=.t.
    end
    ...
    while( positioned  )
        ? model:get(iter1)
        if( model:iter_children(iter2,iter1) )
            dump(model,iter2) //rekurzió
        end
        positioned:=model:iter_next(iter1)
    end
