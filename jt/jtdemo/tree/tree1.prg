...
local split,bar,push

    dlg:caption:="Tree Application" 
    dlg:layout:="vbox"

    split:=dlg:add(jtsplitpaneNew())
    split:alignx:="center" //default left
    split:divider:=20
    split:orientation:="hor" 
    split:setitem(1,t:=jttreeNew())
    split:setitem(2,jtlabelNew(""))

    bar:=dlg:add(jttoolbarNew()) //default center
