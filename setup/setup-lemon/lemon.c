...
struct lemon {
  ...
  int rrconflict;          /* Number of reduce/reduce conflicts */
  int srconflict;          /* Number of shift/reduce conflicts */
    ...
    for(ap=stp->ap; ap && ap->next; ap=ap->next){
      for(nap=ap->next; nap && nap->sp==ap->sp; nap=nap->next){
         ...
         ** Figure out which one should be used */
         lemp->nconflict += resolve_conflict(ap,nap,lemp->errsym,lemp);
      }
    }
  }
...
char **argv)
{
  ...
  if( statistics ){
    ...
    printf("                   %d states, %d parser table entries, %d (sr=%d/rr=%d) conflicts\n",
      lem.nstate, lem.tablesize, lem.nconflict, lem.srconflict, lem.rrconflict);
  }
  if( lem.nconflict ){
    fprintf(stderr,"%d (sr=%d/rr=%d) parsing conflicts.\n",lem.nconflict,lem.srconflict,lem.rrconflict);
  }
...
int (*cmp)())
{
  ...
  char *set[LISTSIZE];
  int i;
  offset = (unsigned long)((char*)next-list);
  ...
  while( list ){
    ...
    for(i=0; i<LISTSIZE-1 && set[i]!=0; i++){
      ep = merge(ep,set[i],(void*)cmp,offset);
      set[i] = 0;
    }
    set[i] = ep;
  }
  ep = 0;
  for(i=0; i<LISTSIZE; i++) if( set[i] ) ep = merge(ep,set[i],(void*)cmp,offset);
  return ep;
}
      ...
      case OPT_FDBL:
        ...
        if( *end ){
          if( err ){
            ...
            errline(i,(unsigned long)(end-argv[i]),err);
          }
          errcnt++;
        }
        break;
...
int *lineno)
{
  ...
  fprintf(out,"\n#line %d \"%s\"\n",*lineno+2,basename(lemp->outname)); (*lineno)+=2;
  return;
}
...
int *lineno)
{
 ...
 if( sp->type==TERMINAL ){
   cp = lemp->tokendest;
   if( cp==0 ) return;
   fprintf(out,"#line %d \"%s\"\n{",lemp->tokendestln,basename(lemp->filename));
 }else if( sp->destructor ){
   cp = sp->destructor;
   fprintf(out,"#line %d \"%s\"\n{",sp->destructorln,basename(lemp->filename));
 }else if( lemp->vardest ){
   cp = lemp->vardest;
   if( cp==0 ) return;
   fprintf(out,"#line %d \"%s\"\n{",lemp->vardestln,basename(lemp->filename));
 }
 for(; *cp; cp++){
   if( *cp=='$' && cp[1]=='$' ){
...
int *lineno)
{
 ...
 if( rp->code ){
   fprintf(out,"#line %d \"%s\"\n{",rp->line,basename(lemp->filename));
