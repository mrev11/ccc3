...
local infile:="cbc.prg"

local key:=a"proba szerencse"
local ks:=crypto_blowfish_set_key(key)
...
local text,blk,blksiz:=128 //8 többszöröse kell legyen
    ...
    while( len(text)>0  )  
        ...
        enctext+=crypto_blowfish_cbc_encrypt(blk,ks,@ivec,.t.)
        hexdump(ivec)
    end
    ...
    while( len(text)>0  )  
        blk:=left(text,blksiz)
        text:=substr(text,blksiz+1)
        dectext+=crypto_blowfish_cbc_encrypt(blk,ks,@ivec,.f.)
        hexdump(ivec)
    end
