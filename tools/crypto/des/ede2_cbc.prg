...
#ifdef NOT_DEFINED
  The DES_ede2_cbc_encrypt() macro implements two-key Triple-DES 
...
local text,blk,blksiz:=64 //8 többszöröse kell legyen
    ...
    while( len(text)>0  )  
        ...
        enctext+=crypto_des_ede2_cbc_encrypt(blk,ks1,ks2,@ivec,.t.)
        hexdump(ivec)
    end
    ...
    while( len(text)>0  )  
        blk:=left(text,blksiz)
        text:=substr(text,blksiz+1)
        dectext+=crypto_des_ede2_cbc_encrypt(blk,ks1,ks2,@ivec,.f.)
        hexdump(ivec)
    end
