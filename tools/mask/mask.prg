...
static maskfile:=""
static origscrn:=NIL
static origsize:=NIL    //a sizeCCCxRRR-ek közül az egyik
...
static termsize:=NIL    //a sizeCCCxRRR-ek közül az egyik
    ...
    if( !empty(maskfile) )
        ...
        if( termsize!=NIL )
            ...
            if( len(origscrn)<=CALCSIZE(termsize) )
                //nagyobba tölti be, mint az eredeti mérete
                restscreen(0,0,origsize[2]-1,origsize[1]-1,origscrn)
                origscrn:=savescreen(0,0,MAXROW,MAXCOL)

            else
                //kisebbe tölti be, mint az eredeti mérete
                for n:=0 to MAXROW
                    line:=origscrn::substr(n*origsize[1]*4+1,origsize[1]*4)::left(termsize[1]*4)
            ...
            //akkorába tölti be, mint amekkora a mérete
            termsize:=origsize
            termsetup()
        ...
        if( termsize==NIL )
            termsize:=size80x25 //régi default
        end
        termsetup()
        clear screen
        origscrn:=savescreen(0,0,MAXROW,MAXCOL)
    ...
    while( .t. )
        ...
        elseif( key==KEY_LOAD )
            ...
            if(!empty(aFile))
                ...
                //terminál méret változás
                if( len(origscrn)!=SCREENSIZE )
                    origsize:=termrect(len(origscrn))
                    termsize:=origsize
                    termsetup()
                end
