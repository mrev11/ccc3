...
class  tableentity(object) new: //nem kell new
    ...
    method  __columnrefnew__            {|this,name,expr|sql2.db2.columnrefNew(name,expr)}
    method  __columndefnew__            {|this,name,expr,type|sql2.db2.columndefNew(name,expr,type)}
    method  __indexdefnew__             {|this,name,seg,uni|sql2.db2.indexdefNew(name,seg,uni)}
...
****************************************************************************
static function tabname_i(this,x)  //create index-be írandó név
...
//    if( (pos:=rat(".",name))>0 )
//       name:=substr(name,pos+1) //schema levágva
//    end
    return  name
    ...
    DB2-ben minősíteni kell az index nevet, azaz kell a séma.
#endif
...
local rowset:=sql2.db2.rowsetNew(this)
...
local clid,n,c,t,buflen:=0,memcnt:=0
    ...
    clid:=classRegister(this:__rowclassname__,{sql2.db2.rowentityClass()}) 
