...
local fldcnt
    ...
    stmt+=" from "+sql2.mysql.sqlbind(this:__table__:__fromclause__,bindarray)

    if( filterclause!=NIL )
        stmt+=" "+sql2.mysql.sqlbind(filterclause,bindarray)
    end
...
//        stmt+=" for update " //nem lehet timeout-ot megadni
    ...
    //stmt:=sql2.mysql.sqlidcase(stmt,.f.)  //megszűnt: 2011.07.20
    
    sql2.mysql.sqldebug(stmt)

    this:__stmthandle__:=sql2.mysql._my_real_query(this:__table__:connection:__conhandle__,stmt)

    if( this:__stmthandle__==NIL ) 
        //? "hibás, nincs letárolva"
        err:=sql2.mysql.sqlerrorCreate(this:__table__:connection:__conhandle__)
...
local clause,time
    ...
    elseif( valtype(wait)=="N" )
        clause:=" for update"  //timeout/nowait not supported
        ...
        //time:=wait[2..] //timeout not supported
    end
