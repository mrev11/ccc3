...
class  tableentity(object) new: //nem kell new
    ...
    method  __columnrefnew__            {|this,name,expr|sql2.postgres.columnrefNew(name,expr)}
    method  __columndefnew__            {|this,name,expr,type|sql2.postgres.columndefNew(name,expr,type)}
    method  __indexdefnew__             {|this,name,seg,uni|sql2.postgres.indexdefNew(name,seg,uni)}
...
local x,n,t,p,p1,a,q
    ...
    for n:=1 to len(this:tablist)
        ...
        if( p!=0 )
            ...
            t:=left(t,p-1)
            q:=quote(t)+" as "+a
            p1:=sql2.postgres.sqltabalias(x,a)
...
local rowset:=sql2.postgres.rowsetNew(this)
...
local whereclause:=""
    ...
    for n:=1 to len(key)
        ...
        nam:=col:expr
        seg:=sql2.postgres.sqlliteral(key[n])
...
local clid,n,c,t,buflen:=0,memcnt:=0
    ...
    clid:=classRegister(this:__rowclassname__,{sql2.postgres.rowentityClass()}) 
...
static function init_bool(buffer,offs,x)
    xvputint(buffer,offs,0) //not null
    xvputint(buffer,offs+4,if(x$BINLIT("tT"),1,0))
    return x
...
local buffer
    buffer:=rowent:__buffer__
    xvputint(buffer,offs,0) //not null
    rowent:__memolist__[mempos]:=sql2.postgres._pq_unescapebytea(x)
    return x
