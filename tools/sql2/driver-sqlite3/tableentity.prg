...
class  tableentity(object) new: //nem kell new
    ...
    method  __columnrefnew__            {|this,name,expr|sql2.sqlite3.columnrefNew(name,expr)}
    method  __columndefnew__            {|this,name,expr,type|sql2.sqlite3.columndefNew(name,expr,type)}
    method  __indexdefnew__             {|this,name,seg,uni|sql2.sqlite3.indexdefNew(name,seg,uni)}
...
local link,pos:=at("@",x)
    ...
    x:=sql2.sqlite3.quotedname(x)
    return x+link

****************************************************************************
static function tabname_i(this)  // schema.tabname@dblink=alias -> tabname
...
local pos:=at("=",name)
    ...
    name:=if(pos==0,name,left(name,pos-1))
    pos:=rat(".",name)
    name:=if(pos==0,name,substr(name,pos+1))
    return  name
...
****************************************************************************
static function tabname_q(this) // schema.tabname@dblink=alias -> schema.tabname
local name:=this:tablist[1]
local pos:=at("=",name)
...
local x,n,t,p,p1,a,q
    ...
    for n:=1 to len(this:tablist)
        ...
        if( p!=0 )
            ...
            t:=left(t,p-1)
            q:=quote(t)+" as "+a
            p1:=sql2.sqlite3.sqltabalias(x,a)
...
local rowset:=sql2.sqlite3.rowsetNew(this)
...
local stmt,c,i,n,s,sep
    ...
    for n:=1 to len(this:__indexlist__)
        ...
        stmt+=tabname_q(this)+"_"+i:name+" on "+tabname_i(this)  // tabname_q tabname_i fordítva
...
local whereclause:=""
    ...
    for n:=1 to len(key)
        ...
        nam:=col:expr
        seg:=sql2.sqlite3.sqlliteral(key[n])
...
local clid,n,c,t,buflen:=0,memcnt:=0,dec

    clid:=classRegister(this:__rowclassname__,{sql2.sqlite3.rowentityClass()}) 
    ...
    for n:=1 to len(this:columnlist)
        ...
        elseif( t=="N" )
            c:type::=strtran(".",",") //pont helyére vessző
            dec:=at(",",c:type)
            if( dec>0 )
                dec:=val(substr(c:type,dec+1))
            end
            
            c:offset:=buflen
            c:size:=4+8 //dupla lebegőpontos 
            c:block:=mkblk_n(buflen,dec)
...
****************************************************************************
static function mkblk_n(offs,dec)
local b:={|rowent,x|io_float(rowent:__buffer__,offs,x,dec)}
    return b

static function io_float(buffer,offs,x,dec)
    ...
    if( x==NIL )
        ...
        xvputint(buffer,offs,0) //not null
        xvputflag(buffer,offs+2,.f.) //hot
        xvputdouble(buffer,offs+4,x::round(dec))
    end
...
static function init_float(buffer,offs,x)
    xvputint(buffer,offs,0) //not null
  //xvputdouble(buffer,offs+4,val(x))   //rowset:next adhatná X-ben is 
    xvputdouble(buffer,offs+4,x)        //rowset:next adhatja számként 
    return x
...
static function init_bool(buffer,offs,x)
    xvputint(buffer,offs,0) //not null
  //xvputint(buffer,offs+4,if(x$BINLIT("0"),0,1))  //rowset:next adhatná X-ben is
    xvputint(buffer,offs+4,if(x==0,0,1))           //rowset:next adhatja számként
    return x
