...
#ifdef NOT_DEFINED
  Ebben a modulban a hash itemek {key,...} alakúak,
  tehát egy array, aminek az első eleme a kulcs.

  Könnyen át lehet alakítani, kézenfekvő lehetőségek:
    a) {key,value}
    b) object, object:key a kulcs
    c) stb.
    
  Ilyen átalakítással készült a simplehash osztály,
  {key,value} + hashitem,set,get metódusok, 
  ami bekerült a CCC könyvtárba, és amivel működik 
  az asszociatív tömb indexelés: hash["key"]:=value

  Ha az itteni forrást átviszed egy projektbe, akkor célszerű 
  a projektre jellemző névteret választani (a tutor helyett), 
  hogy a különféle hashtable-k ne ütközzenek, vagy használd 
  a könyvtári simplehash-t.
#endif
