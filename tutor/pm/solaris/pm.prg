...
local b, sig:={}, ord:={}
    ...
    aadd(ord,{"Process ID",{||order:="pid",reload(b)}})
    aadd(ord,{"CPU usage",{||order:="cpu",reload(b)}})
    aadd(ord,{"Time of execution",{||order:="time",reload(b)}})
    aadd(ord,{"Process name",{||order:="name",reload(b)}})
    aadd(ord,{"Owner of process",{||order:="owner",reload(b)}})
    brwMenu(b,"Order","Set process order",ord)
...
#define NUM(x) val(substr(x[1],offs))
#define STR(x) substr(x[1],offs)
...
local t, n, offs
    ...
    elseif( order=="cpu" )
        offs:=at("%",t[1][1])-4
        asort(t,,,{|x,y|NUM(x)>NUM(y)}) //CPU sorrend
    elseif( order=="time" )
        offs:=at("%",t[1][1])-12
        asort(t,,,{|x,y|STR(x)>STR(y)}) //TIME sorrend
    elseif( order=="name" )
        offs:=at("%",t[1][1])+1
        asort(t,,,{|x,y|STR(x)<STR(y)}) //NAME sorrend
    elseif( order=="owner" )
        offs:=7
        asort(t,,,{|x,y|STR(x)<STR(y)}) //OWNER sorrend
    else
        asort(t,,,{|x,y|x[1]<y[1]}) //pid sorrend
    end
